---
title: 09｜从零实现一个角色扮演的聊天机器人
date: 2026-09-23
source: https://time.geekbang.org/column/article/826606
tags:
  - clippings
  - LangChain
  - 聊天机器人
...
> [!ABSTRACT] 快速概览
> 从命令行聊天机器人起步，用 ChatMessageHistory + RunnableWithMessageHistory 补上无状态 API 缺失的会话记忆，再结合 PromptTemplate（含 MessagesPlaceholder）实现角色扮演（AI 孔子），并用 trim_messages + tiktoken 限制消息规模。

---

## 📝 正文内容

<audio title="09｜从零实现一个角色扮演的聊天机器人" src="https://res001.geekbang.org/media/tts_audio/20250928/tts-3255-4-826466/ld/ld.m3u8"></audio>
...
你会发现我们传给 RunnableWithMessageHistory 的参数是一个函数，也就是这里的 get_session_history，正如它的名字所示，它会获取会话的历史。如果我们在开发的是一个多人聊天的应用，每个人就是一个不同的会话，在聊天时，我们需要获取到自己的聊天历史。在我们这个实现里，获取聊天历史的依据是 session_id。get_session_history 的实现很简单，如果不存在这个 session_id 对应的聊天历史，就创建一个新的，如果存在，就返回已有的。
...
这其中最关键的是就是 max_tokens，这也是我们限制消息规模的主要原因。不过，这里是按照 Token 进行计算，我们该怎么计算 Token 呢？对 OpenAI API 来说，一种常见的解决方案是采用 tiktoken，这是一个专门用来处理的 Token 的程序库。下面是采用了 tiktoken 的计算 Token 的实现：
...
- [ ] 跑通最简命令行聊天机器人，复现“记不住上下文”问题
- [ ] 用 RunnableWithMessageHistory + InMemoryChatMessageHistory 实现带 session_id 的会话记忆
- [ ] 加上角色扮演 PromptTemplate（含 MessagesPlaceholder），实现一个自定义角色机器人
- [ ] 接入 trim_messages 与 tiktoken_counter，限制 max_tokens 并保留系统角色提示

## 🔗 关联笔记

- [[08｜LangChain：核心抽象]] | 核心抽象（08）
- [[07｜LangChain：一个AI应用开发生态]] | LangChain 生态（07）
- [[04｜提示工程：更好地释放LLM的能力]] | 提示工程（04）

## 📎 参考资料

- 极客时间《程序员的 AI 开发第一课》第 09 讲 — 郑晔
- tiktoken
...
> [!TIP]
> 实现一个聊天机器人的关键点就是管理好聊天历史——ChatMessageHistory 记事，PromptTemplate 定角色，trim_messages 控规模。
