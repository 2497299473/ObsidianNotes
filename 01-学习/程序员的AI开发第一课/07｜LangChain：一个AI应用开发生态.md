---
title: 07｜LangChain：一个AI应用开发生态
date: 2026-09-23
source: https://time.geekbang.org/column/article/825266
tags:
  - clippings
  - LangChain
  - LCEL
...
> [!ABSTRACT] 快速概览
> 把 LangChain 从“开发框架”扩展理解为完整生态——开发框架（基础抽象 + LCEL）、社区生态（各类实现与提示词社区）和扩展生态（LangServe / LangSmith / LangGraph），并讨论是否选择 LangChain、以及如何把它当作学习材料。

---

## 📝 正文内容

<audio title="07｜LangChain：一个AI应用开发生态" src="https://res001.geekbang.org/media/tts_audio/20250928/tts-3248-4-825236/ld/ld.m3u8"></audio>
...
除了大模型，其它基础抽象都有很多实现，比如，向量数据库，有 Milvus、FAISS、Qdrant、Chroma 等等，文档加载既可以加载一个 Web 页面，也可以从诸如 S3、微软 OneDrive 加载，甚至从 Twitter、Telegram 加载内容。总之，针对不同的抽象，都有对应的实现，你可以根据自己的需要在 LangChain 的 集成 页面上找到对应的内容。

除了直接的代码实现，还有各种提示词模板。相比于其它的代码，提示词模板都是纯文本。LangChain 社区有一个专门的 提示词社区 。在这里，可以找到各种类别的提示词：实现 Agent 、代码编写、文档问答、内容总结，主流的提示词在这里都可以找到。而在编写代码的过程中，我们只要把它加载过来，就可以形成一个提示词模板：
...
作为开发框架，LangChain 提供了一些开发应用所需的基础抽象和 LangChain 表达式语言。基础抽象只是一些接口，大部分实现都是由社区生态提供的。我们可以根据自己的需要在 LangChain 的 集成 页面上找到需要的内容。除此之外，LangChain 还有一个专门的 提示词社区 ，存放各种提示词。开发框架和社区生态内容都是为了构建链，LangChain 团队还构建了其它一些应用：LangServe、LangSmith 和 LangGraph。
...
这一讲我们概览了 LangChain 整个生态。对我们日常工作来说，通常是要到社区中寻找我们需要的东西。我建议你到 集成 页面上去转转，看看有哪些东西，可能你会在这个页面上发现很多你不知道的东西，欢迎在留言区分享你的意外收获。

## ✅ 待办事项

- [ ] 打开 LangChain 集成页，列出自己项目可能用到的 3 个实现
- [ ] 用 LCEL 写一条 model | parser 的最小链并 invoke
- [ ] 从提示词社区拉取一个 ReAct 模板，对照理解工具调用提示词结构
- [ ] 写一份“是否选用 LangChain”的决策清单（模型抽象 / 记忆 / 模板是否多余）

## 🔗 关联笔记

- [[06｜OpenAI API：LLM编程的事实标准（下）]] | 聊天应答（06）
- [[08｜LangChain：核心抽象]] | 核心抽象（08）
- [[04｜提示工程：更好地释放LLM的能力]] | 提示工程（04）

## 📎 参考资料

- 极客时间《程序员的 AI 开发第一课》第 07 讲 — 郑晔
- LangChain 集成
- LangChain 提示词社区
- 《软件设计之美》11 | DSL：你也可以设计一门自己的语言
...
> [!TIP]
> LangChain 不只是一个框架，而是一个生态系统——框架定抽象，社区供实现，扩展管部署与观测；是否选用按需评估，同时它也是极好的学习材料。
