---
title: 06｜OpenAI API：LLM编程的事实标准（下）
date: 2026-09-23
source: https://time.geekbang.org/column/article/824742
tags:
  - clippings
  - OpenAI API
  - 流式应答
...
> [!ABSTRACT] 快速概览
> 书接上文讲解聊天补全接口的应答：标准 HTTP 应答结构（choices / message / tool_calls / logprobs）与流式应答模式，并说明流式背后的 SSE 技术、报文格式，以及它与 WebSocket 的取舍。

---

## 📝 正文内容

<audio title="06｜OpenAI API：LLM编程的事实标准（下）" src="https://res001.geekbang.org/media/tts_audio/20250928/tts-3247-4-824660/ld/ld.m3u8"></audio>
...
![](https://static001.geekbang.org/resource/image/a3/a7/a3769260afyy5ff7025befb0e58a4ca7.gif?wh=1252x552)
...
- [ ] 抓取一次完整的聊天补全 JSON 应答，对照 id / choices / usage 字段做标注
- [ ] 在请求中打开 logprobs 与 top_logprobs，观察 token 概率返回结构
- [ ] 分别用常规应答与 stream=true 调用同一提示词，对比首字延迟
- [ ] 读一段 SSE 纯数据消息报文，解释为何 OpenAI 在 POST 上用 SSE 仍被接受

## 🔗 关联笔记

- [[05｜OpenAI API：LLM编程的事实标准（上）]] | 请求参数（05）
- [[04｜提示工程：更好地释放LLM的能力]] | 提示工程（04）
- [[07｜LangChain：一个AI应用开发生态]] | LangChain 生态（07）

## 📎 参考资料

- 极客时间《程序员的 AI 开发第一课》第 06 讲 — 郑晔
...
> [!TIP]
> 聊天应答的核心是消息内容；聊天场景用流式应答（SSE）抢首字体验，推理引擎场景用常规 HTTP 应答即可。
