---
title: 19｜如何在项目中使用开源模型
date: 2026-09-23
source: https://time.geekbang.org/column/article/832429
tags:
  - clippings
  - 开源模型
  - LangChain
...
> [!ABSTRACT] 快速概览
> 项目里同时用开源模型和供应商模型时的两种封装：LangChain（langchain-huggingface 把 HF 模型嵌入抽象）和集中接入（One-API 接 Ollama 本地服务）。覆盖 Ollama 安装拉取模型、自带/OpenAI 兼容 API，以及从 Hugging Face 仓库运行模型的条件。

---

## 📝 正文内容

<audio title="19｜如何在项目中使用开源模型" src="https://res001.geekbang.org/media/tts_audio/20250929/tts-3264-4-913811/ld/ld.m3u8"></audio>
...
你现在知道了，既然 Ollama 提供了服务，One API 当然可以通过服务接口连接它。配置 Ollama 服务，主要是根据自己的需要配置所需的模型。
...
![](https://static001.geekbang.org/resource/image/d4/ac/d446227dd3fb14ced7432f37f31eb0ac.gif?wh=1878x1244)
...
- [ ] 安装 langchain-huggingface，用 HuggingFacePipeline + ChatHuggingFace 跑通一次 invoke
- [ ] 安装 Ollama，pull 并 run 一个 qwen2.5 版本，体验本地对话
- [ ] 在 One-API 中添加 Ollama 渠道，用 OpenAI SDK 指定本地模型名调用
- [ ] 在 Hugging Face 模型页确认 “Use this model” 是否含 Ollama，试跑 `ollama run hf.co/...`

## 🔗 关联笔记

- [[18｜开源模型：如何使用Hugging Face上的模型]] | 开源模型（18）
- [[17｜集中接入：将大模型统一管理起来]] | 集中接入（17）
- [[20｜模型微调：创建一个属于自己的大模型]] | 模型微调（20）
- [[08｜LangChain：核心抽象]] | LangChain（08）
- [[04｜提示工程：更好地释放LLM的能力]] | 提示工程（04）

## 📎 参考资料

- 极客时间《程序员的 AI 开发第一课》第 19 讲 — 郑晔
- langchain-huggingface（Hugging Face 与 LangChain 共同维护）
- Ollama（本地运行大模型的轻量级框架；安装脚本见 `https://ollama.com/install.sh`）
- vllm、gpt4all、Cline（原 Claude Dev）
...
> [!TIP]
> 开源模型落地要么在代码里用 LangChain 抽象封装，要么丢给大模型代理；本地跑 Ollama 既省钱又不丢数据，还逼你真正理解大模型。
