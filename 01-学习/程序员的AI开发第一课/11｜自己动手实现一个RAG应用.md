---
title: 11｜自己动手实现一个RAG应用
date: 2026-09-23
source: https://time.geekbang.org/column/article/827564
tags:
  - clippings
  - RAG
  - LangChain
...
> [!ABSTRACT] 快速概览
> 基于 LangChain 动手实现一个 RAG 应用：索引侧用 DocumentLoader / TextSplitter / Chroma / OpenAIEmbeddings 把资料向量化入库，检索生成侧用 Retriever + 提示词模板 + 消息历史拼成完整链路，并解释 `|` 组链与 `RunnablePassthrough.assign` 的实现细节。

---

## 📝 正文内容

<audio title="11｜自己动手实现一个RAG应用" src="https://res001.geekbang.org/media/tts_audio/20250928/tts-3254-4-827560/ld/ld.m3u8"></audio>
...
我们 上一讲 已经讲过了索引的基本过程，你可以先回顾一下，这样结合代码看起来就比较容易理解了：
...
我们这里用的 similarity_search 表示的是根据相似度进行搜索，还可以使用 max_marginal_relevance_search，它会采用 MMR（Maximal Marginal Relevance，最大边际相关性）算法。这个算法可以在保持结果相关性的同时，尽量选择与已选结果不相似的内容，以增加结果的多样性。
...
文档检索出来，并不能直接就和我们的问题拼装到一起。这时，就轮到提示词登场了。下面是我们在代码里用到的提示词（改造自 https://smith.langchain.com/hub/rlm/rag-prompt ）
...
- [ ] 跑通索引代码，把开篇词写入本地 Chroma 向量库
- [ ] 用 similarity_search 与 max_marginal_relevance_search 各检索一次，对比结果差异
- [ ] 把业务资料替换进 TextLoader，验证检索生成效果
- [ ] 读懂 `itemgetter | retriever | format_docs` 与 `RunnablePassthrough.assign` 的组链原理

## 🔗 关联笔记

- [[10｜RAG：让大模型知道更多东西]] | RAG 概念（10）
- [[12｜Agent：从聊天到工作]] | 下一讲（12）
- [[04｜提示工程：更好地释放LLM的能力]] | 提示工程（04）
- [[02｜技术视角：你应该知道的LLM基础知识]] | 向量基础（02）

## 📎 参考资料

- 极客时间《程序员的 AI 开发第一课》第 11 讲 — 郑晔
...
> [!TIP]
> LangChain 为 RAG 应用提供了基础的支持：DocumentLoader 进料、TextSplitter 切块、VectorStore 存取、Retriever 检索，再用提示词把上下文拼进生成链路。
