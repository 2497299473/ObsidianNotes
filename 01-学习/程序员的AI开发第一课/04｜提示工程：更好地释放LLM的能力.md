---
title: 04｜提示工程：更好地释放LLM的能力
date: 2026-09-23
source: https://time.geekbang.org/column/article/822235
tags:
  - clippings
  - 提示工程
  - Prompt Engineering
...
> [!ABSTRACT] 快速概览
> 面向应用开发的提示工程：在提示词公式之上扩展四类技术——零样本提示（通用任务）、少样本提示（给例子做特定任务）、思维链提示（让模型慢下来做推理），以及 ReAct 框架（推理 + 行动，接入工具形成 Agent 雏形）。

---

## 📝 正文内容

<audio title="04｜提示工程：更好地释放LLM的能力" src="https://res001.geekbang.org/media/tts_audio/20250928/tts-3246-4-822153/ld/ld.m3u8"></audio>
...
成绩不足 60 分的，复习本学期课程内容

有了这些例子，我们就可以要求大模型回答我们具体的问题，也就是：

小刚的期末成绩是 74 分，他的假期活动应该是什么？

大模型参考我们给出的例子，进行推理，得出了相应的结论：

小刚的期末成绩是 74 分，他的假期活动应该是完成假期作业。
...
思维链提示就是这样一种技术，我们来看一个例子，这个例子来自将思维链介绍给行业的 论文 ：
...
ReAct 框架就是在这个想法下诞生的。ReAct 实际上是两个单词的缩写：Reasoning + Acting，也就是推理 + 行动。下面是这个例子来自引入 ReAct 框架的 论文 ，其原始问题是：
...
如果单看这个例子，你还可能会有些困惑，那下面的例子可以帮助你更好地理解 ReAct 是一个框架。这是来自 LangChain 社区的 一个提示词模板 ，后面我们会讲到 LangChain，这里我们先把注意力放到提示词本身：
...
到这里，我们已经快速了解了最基本的几个提示技术。显然，这不是提示技术全部。如果你有兴趣了解，可以到 提示工程指南 这个网站上去了解更多。
...
我们这里只提到了基本的几项提示技术，算是抛砖引玉。希望你可以到 提示工程指南 学习一项提示技术，然后，在评论区分享你的理解。

## ✅ 待办事项

- [ ] 各写一条零样本 / 少样本 / 思维链提示词并横向对比
- [ ] 在复杂推理题上加 “Let’s think step by step” 验证思维链效果
- [ ] 精读文末 ReAct 提示词模板，画出 Thought/Action/Observation 循环

## 🔗 关联笔记

- [[03｜提示词：怎样与大模型沟通？]] | 提示词（03）
- [[02｜技术视角：你应该知道的LLM基础知识]] | 技术视角（02）
- [[开篇词｜普通程序员如何迈进AI时代？]] | 开篇词

## 📎 参考资料

- 极客时间《程序员的 AI 开发第一课》第 04 讲 — 郑晔
...
> [!TIP]
> 提示工程是为了引导大模型给出更好的答案：零样本打底，少样本定域，思维链补推理，ReAct 接工具突破纯文本边界。
