---
title: 10｜RAG：让大模型知道更多东西
date: 2026-09-23
source: https://time.geekbang.org/column/article/827289
tags:
  - clippings
  - RAG
  - 向量数据库
...
> [!ABSTRACT] 快速概览
> 大模型只懂通用知识、不懂你的业务数据。RAG（检索增强生成）用“本地检索 → 拼进提示词 → 再生成”补齐这块短板，覆盖处理流程、向量与向量数据库、索引过程，以及混合搜索、Rerank、知识图谱 RAG 等演进方向。

---

## 📝 正文内容

<audio title="10｜RAG：让大模型知道更多东西" src="https://res001.geekbang.org/media/tts_audio/20250928/tts-3253-4-827226/ld/ld.m3u8"></audio>
...
我们在 02 讲 学习过向量，我们说过，许多 AI 算法处理的都不是文字，而是向量。采用向量的方案，“语义”的匹配程度就转换成了向量之间的相似程度。计算向量相似度的算法有很多，比如，余弦相似度、内积、欧氏距离等等。
...
- [ ] 调研现有 RAG 技术的主要问题与对应解法，整理成对比笔记
- [ ] 对比 SQL like 字面匹配与向量语义匹配，写清各自适用场景
- [ ] 画出索引四步（提取 → 拆分 → Embedding → 入库）流程图
- [ ] 了解混合搜索 / Rerank / 知识图谱 RAG 各自解决什么痛点

## 🔗 关联笔记

- [[09｜从零实现一个角色扮演的聊天机器人]] | 上一讲（09）
- [[11｜自己动手实现一个RAG应用]] | 动手实现（11）
- [[02｜技术视角：你应该知道的LLM基础知识]] | 向量基础（02）
- [[04｜提示工程：更好地释放LLM的能力]] | 提示工程（04）

## 📎 参考资料

- 极客时间《程序员的 AI 开发第一课》第 10 讲 — 郑晔
...
> [!TIP]
> RAG 是为了让大模型知道更多的东西：本地检索业务知识，拼进提示词再生成；向量 + 向量数据库撑起语义匹配，索引与检索一体两面。
