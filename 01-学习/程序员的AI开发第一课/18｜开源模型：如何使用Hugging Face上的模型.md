---
title: 18｜开源模型：如何使用Hugging Face上的模型
date: 2026-09-23
source: https://time.geekbang.org/column/article/831599
tags:
  - clippings
  - 开源模型
  - Hugging Face
...
> [!ABSTRACT] 快速概览
> 为满足私有化部署、数据不出内网的需求，转向 Hugging Face 上的开源模型：高层 pipeline 与底层 Tokenizer/Model 两种调用方式，并说明 Token ↔ 文本三步流程，最后用 Streamer / TextIteratorStreamer 在高层与底层接口上实现流式输出。

---

## 📝 正文内容

<audio title="18｜开源模型：如何使用Hugging Face上的模型" src="https://res001.geekbang.org/media/tts_audio/20250929/tts-3263-4-913807/ld/ld.m3u8"></audio>
...
还记得我们在 02 讲介绍过的大模型处理流程吗？基本上就是这样一个过程。因为过程中涉及到了 Token 和文本之间的转换，所以，这里还有一个 Tokenizer，它就是负责转换的模型。一般来说，大模型都要和对应的 Tokenizer 一起使用，所以，你会看到它俩往往给出的是同一个模型名字：
...
- [ ] 用 pipeline 跑通 Qwen2.5-0.5B-Instruct 文本生成示例
- [ ] 用底层 AutoTokenizer + AutoModelForCausalLM 复现同一功能，对照三步流程
- [ ] 加 TextStreamer / TextIteratorStreamer 实现流式输出
- [ ] 配置 HF_ENDPOINT 镜像与 Access Token，浏览不同用途（translation 等）的模型

## 🔗 关联笔记

- [[17｜集中接入：将大模型统一管理起来]] | 集中接入（17）
- [[19｜如何在项目中使用开源模型]] | 开源模型落地（19）
- [[02｜技术视角：你应该知道的LLM基础知识]] | 技术视角（02）
- [[01｜用户视角：你应该知道的LLM基础知识]] | 用户视角（01）

## 📎 参考资料

- 极客时间《程序员的 AI 开发第一课》第 18 讲 — 郑晔
- Hugging Face（模型 / 数据集 / Space 开源社区）
- transformers 程序库、Gradio
...
> [!TIP]
> Hugging Face 上的模型可私有化部署：pipeline 易用，底层三步（Token 化 → 生成 → 解码）灵活，Streamer 补上流式输出。
