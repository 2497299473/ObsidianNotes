---
title: 08｜LangChain：核心抽象
date: 2026-09-23
source: https://time.geekbang.org/column/article/825655
tags:
  - clippings
  - LangChain
  - ChatModel
...
> [!ABSTRACT] 快速概览
> 深入 LangChain 三大核心抽象：ChatModel 负责生成、PromptTemplate 处理输入并拆分开发者/用户提示词、OutputParser 处理输出并内置格式指令；用 LCEL 管道组装成链，跑通翻译与结构化 JSON 输出示例。

---

## 📝 正文内容

<audio title="08｜LangChain：核心抽象" src="https://res001.geekbang.org/media/tts_audio/20250928/tts-3249-4-825617/ld/ld.m3u8"></audio>
...
引入了 PromptTemplate 之后，开发者写的提示词和用户的消息就完全分开了。开发者可以不断地调整提示词以便达到更好的效果，而这一切对用户完全屏蔽掉了。此外，还有一个好处，就是好的提示词模板是可以共享出来的，我们甚至可以把别人写好的提示词用在自己的代码里。我们在上一讲说过的 提示词社区 就是这样诞生的。
...
- [ ] 跑通 ChatOpenAI 同步 invoke 与 stream 两个示例
- [ ] 用 ChatPromptTemplate 改写一条硬编码提示词，验证占位符替换
- [ ] 实现一条 prompt | model | parser 链，分别接 StrOutputParser 与 JsonOutputParser
- [ ] 调用 parser.get_format_instructions()，看格式指令实际写入了什么提示词

## 🔗 关联笔记

- [[07｜LangChain：一个AI应用开发生态]] | LangChain 生态（07）
- [[09｜从零实现一个角色扮演的聊天机器人]] | 聊天机器人（09）
- [[05｜OpenAI API：LLM编程的事实标准（上）]] | OpenAI API（05）

## 📎 参考资料

- 极客时间《程序员的 AI 开发第一课》第 08 讲 — 郑晔
- LangChain 提示词社区
...
> [!TIP]
> ChatModel 是核心，PromptTemplate 处理输入，OutputParser 处理输出——三者用 LCEL 管道串成链，就是一条最小可用的大模型应用。
