---
title: 05｜OpenAI API：LLM编程的事实标准（上）
date: 2026-09-23
source: https://time.geekbang.org/column/article/824312
tags:
  - clippings
  - OpenAI API
  - Chat Completions
...
> [!ABSTRACT] 快速概览
> 解释为何选 OpenAI API 作为 LLM 编程的事实标准，并以最核心的聊天补全接口为例，通览请求参数的四类划分——核心参数（model / messages）、工程参数、工具参数与模型参数。

---

## 📝 正文内容

<audio title="05｜OpenAI API：LLM编程的事实标准（上）" src="https://res001.geekbang.org/media/tts_audio/20250928/tts-3250-4-822707/ld/ld.m3u8"></audio>
...
response_format，应答格式。缺省情况下，这个接口只生成文本内容。但对开发来说，我们经常会用到 JSON 格式。我们当然可以用提示词要求大模型返回，也可以通过设置 response_format 让 API 直接返回 JSON 格式，具体做法可以参考 OpenAI 的 结构化输出 。
...
- [ ] 用 curl 或 SDK 调通 /v1/chat/completions，确认 model 与 messages 两个核心参数
- [ ] 为同一条提示词分别设置 temperature / max_completion_tokens，对比输出与成本
- [ ] 阅读 tools 与 tool_choice 说明，写一个可调用工具的请求示例
- [ ] 通读模型参数（seed / top_p / frequency_penalty），标出自己真正会用的
...
> [!TIP]
> 聊天补全接口的请求参数中，最核心的是模型和消息列表——先吃透核心参数，再按需深入工具参数做 Agent。
