---
title: 14｜用LangChain实现一个Agent
date: 2026-09-23
source: https://time.geekbang.org/column/article/829302
tags:
  - clippings
  - Agent
  - LangChain
...
> [!ABSTRACT] 快速概览
> 用 LangChain 重建上一讲的 ReAct Agent：`@tool` 把函数变成工具，通用 ReAct 提示词模板配合 `create_react_agent` + `AgentExecutor` 跑循环；并区分 Tool 与 Toolkit，演示 ShellTool、GitHub Toolkit 等现成集成。

---

## 📝 正文内容

<audio title="14｜用LangChain实现一个Agent" src="https://res001.geekbang.org/media/tts_audio/20250929/tts-3261-4-829289/ld/ld.m3u8"></audio>
...
接下来就是提示词，这里我们没有把上一讲的提示词直接搬过来，而是采用了一个 更通用的提示词 ：
...
如果你还记得，我们在学习 提示工程 的那一讲的时候，已经见过这个提示词模板了，再结合上一讲介绍过的提示词，对于这里的思考（Thought）、行动（Action）和观察（Observation），你肯定不陌生了。
...
在前面的实现中，为了兼容上一讲的内容，我们自定义了自己的工具实现。实际上， LangChain 社区已经为我们提供了 大量的工具 ，让我们可以很方便地集成各种东西。在很多情况下，这些工具我们只要直接拿过来用就好，下面是一个例子：
...
- [ ] 跑通 @tool 版 calculate / ask_fruit_unit_price，打印 name / description / args 验证提取结果
- [ ] 对照 create_react_agent + AgentExecutor，理清提示词模板与解析逻辑的分工
- [ ] 用 ShellTool 改造代码，完成一次自然语言驱动的文件创建
- [ ] 选一个 LangChain 社区工具（或 Toolkit）组装出新的 Agent

## 🔗 关联笔记

- [[13｜从头实现一个Agent]] | 从头实现（13）
- [[12｜Agent：从聊天到工作]] | Agent 概念（12）
- [[04｜提示工程：更好地释放LLM的能力]] | ReAct 提示词（04）
- [[15｜长期记忆：让大模型更了解你]] | 后续（15）

## 📎 参考资料

- 极客时间《程序员的 AI 开发第一课》第 14 讲 — 郑晔
...
> [!TIP]
> 编写基于 LangChain 的 Agent，关键点是学会工具：@tool 变出工具，Toolkit 攒齐工具包，AgentExecutor 负责循环。
