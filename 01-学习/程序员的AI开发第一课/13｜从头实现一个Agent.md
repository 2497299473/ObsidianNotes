---
title: 13｜从头实现一个Agent
date: 2026-09-23
source: https://time.geekbang.org/column/article/828836
tags:
  - clippings
  - Agent
  - ReAct
...
> [!ABSTRACT] 快速概览
> 抛开 LangChain，用 OpenAI Python 库从零实现一个基于 ReAct 的 Agent：先搭基础聊天机器人类，再写含 Thought / Action / PAUSE / Observation 的提示词，实现动作函数，最后用 `query` 循环把大模型与本地代码串成“自主”执行。

---

## 📝 正文内容

<audio title="13｜从头实现一个Agent" src="https://res001.geekbang.org/media/tts_audio/20250929/tts-3257-4-828501/ld/ld.m3u8"></audio>
...
我们构建这个 Agent 会基于 ReAct 来实现，我们在 04 讲 介绍过，ReAct 表示 Reasoning + Acting，也就是推理和行动。采用这个模式，要经历思考（Thought）、行动（Action）、观察（Observation）三个阶段。

大模型会先思考要做什么，决定采用怎样的行动，然后在环境中执行这个行动，返回一个观察结果。有了这个观察结果，大模型就会重复思考的过程，再次考虑要做什么，采用怎样的行动，这个过程会持续到大模型决定这个过程结束为止。

我们的实现代码参考了 Simon Willison 的一篇 文章 ，这篇文章介绍了如何用 Python 实现 ReAct 模式。
...
ReAct 描述，给大模型解释了 ReAct 的三个阶段：思考（Thought）、行动（Action）、观察（Observation）。这里还多了一个暂停（Pause），其主要的目的就是停下来，这时执行流程就回到我们这里，执行相应的动作。当动作执行完毕，再把控制权返回给大模型。
...
- [ ] 跑通基础 Agent 类，确认 temperature=0 下输出稳定
- [ ] 精读 ReAct 提示词三段结构，标出 Thought / Action / PAUSE / Observation 各自职责
- [ ] 给 Agent 新增至少一个动作函数，并同步写进提示词的 available actions
- [ ] 用 max_turns 观察循环上限如何拦住不收敛的执行

## 🔗 关联笔记

- [[12｜Agent：从聊天到工作]] | Agent 概念（12）
- [[14｜用LangChain实现一个Agent]] | LangChain 实现（14）
- [[04｜提示工程：更好地释放LLM的能力]] | ReAct（04）
- [[03｜提示词：怎样与大模型沟通？]] | 提示词（03）

## 📎 参考资料

- 极客时间《程序员的 AI 开发第一课》第 13 讲 — 郑晔
- Simon Willison：用 Python 实现 ReAct 模式的文章
