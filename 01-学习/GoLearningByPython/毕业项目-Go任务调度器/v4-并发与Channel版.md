...
tags:
  ...
  - Goroutine
  - Channel
description: 毕业项目 v4：引入 goroutine 并发执行任务，用 channel 收集结果，用 context 实现超时控制，用信号量限流。
lark_doc_url: https://my.feishu.cn/docx/HZkedB535o8BCjxB2kEcWKFinTd
---

## 项目目标

**在 v3 基础上**：把顺序执行改为 goroutine 并发执行，用 channel 收集结果，用 context 实现超时控制。

---

## 改造要点

| v3（顺序执行） | v4（并发执行） | 改造维度 |
|---------------|--------------|---------|
| `for _, t := range tasks` 顺序 | `go task.Run()` 并发 | goroutine |
| 直接 append results | 发送到 `chan *Result` | channel |
| 无超时控制 | `context.WithTimeout` | context |
| 无并发数限制 | `sem chan struct{}` 信号量 | worker pool |
...
## Python asyncio → Go 并发 对照

| Python asyncio | Go v4 |
|---------------|-------|
| `asyncio.gather(*tasks)` | `go func()` + `sync.WaitGroup` |
| `asyncio.wait_for(task, timeout)` | `context.WithTimeout` + `select` |
| `asyncio.Semaphore(n)` | `chan struct{}` 信号量 |
| `await task` | `<-results` channel |
| `asyncio.Queue` | `chan *Result` |
...
| 层次 | 检查项 |
|------|--------|
| 🟢 基础 | 能用 goroutine + channel 并发执行任务 |
| 🟡 进阶 | 能用 context + select 实现超时控制 |
| 🔴 挑战 | 能解释为什么用信号量限流，对比 Python asyncio.Semaphore |
...
- ⬅️ 前置：[[v3-接口与错误处理版]]
- ⬅️ 前置：[[05-并发编程-Goroutine与Channel]]
- ➡️ 下一步：[[v5-测试与部署版]]
