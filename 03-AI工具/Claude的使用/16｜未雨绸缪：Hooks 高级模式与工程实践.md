...
created: 2026-06-24
description: 深入探讨 Hooks 高级模式与工程实践，包括 Stop Hook 质量门控、SubagentStart/SubagentStop 事件、frontmatter 内置 Hooks、多 Hook 链、异步/HTTP Hook，以及 Hook 工程设计方法论。
tags:
  - clippings
  - claude-code
  - hooks
lark_doc_url: https://my.feishu.cn/docx/GPg1dRLQ4oCSTmx8r6xcHvrOnre
---
...
子代理定义（`.claude/agents/code-reviewer.md`）+ 全局 `settings.json` 的三层保护：
...
06-Hooks/projects/
├── 01-safety-hooks/
│   ├── .claude/settings.json  ← PreToolUse(Bash) + PreToolUse(Write|Edit) + PostToolUse(*)
│   ├── hooks/
│   │   ├── block-dangerous.sh
│   │   ├── protect-files.sh
│   │   └── audit-log.sh
│   └── README.md
└── 02-quality-hooks/
    ├── .claude/settings.json  ← PostToolUse(Write|Edit) + Stop
    ├── hooks/
    │   ├── auto-format.sh
    │   ├── lint-check.sh
    │   └── run-tests.sh
...
**Hooks vs Pipeline：不是替代，是补位。**

Pipeline 是代码质量保障的终极防线，没有争议。但 Hooks 解决的是 Pipeline 天然覆盖不了的一个时间窗口：**Claude 在本地执行动作的那一秒。**

`rm -rf /data` 执行了就是执行了，Pipeline 跑得再快也追不回被删的数据。

打个比方：Pipeline 是保险，Hooks 是烟雾报警器。保险赔得了钱，赔不了命。
...
去掉任何一层，都会在对应的时间窗口留下空白。尤其是 AI 自动写代码的场景，Hooks 的价值不是「也能跑测试」，而是**把反馈循环从分钟级压缩到毫秒级**。


### 思考题

**Q2：全局 vs 项目级 vs Frontmatter 的适用场景**

| 场景 | 全局配置 | 项目级 | Frontmatter |
|------|:---:|:---:|:---:|
| 跨所有项目通用 | ✅ | ❌ | ❌ |
| 与技术栈相关 | ❌ | ✅ | ✅ |
| 只在特定任务生效 | ❌ | ❌ | ✅ |
...
**Q3：代码提交前自动检查 Hook 方案**

- 事件：PreToolUse，matcher: Bash，拦截 `git commit`/`git push`
- Lint 通过、测试覆盖、敏感信息检查 → **command** 类型（确定性规则，零误判）
- 完整 CR 报告并归档 → **agent** 类型（需要读代码、写报告，防止污染主上下文）
- 提示词类型在这个场景下没有用武之地——检查结果只有过/不过，不需要 LLM 的模糊判断

**Q4：frontmatter 自检 vs SubagentStop 外部验收**

本质是**事中检与事后检**的区别。
- 事中检（frontmatter 自检）→ 只见树木，检查「我输出的有没有漏」
- 事后检（SubagentStop 外部验收）→ 只见森林，判断「它觉得完成了，但其实够不够好」
- 两者视角互补，缺一不可
