...
source: https://time.geekbang.org/column/article/964160
author: 黄佳
published: 2026-04-07
created: 2026-06-28
description: Claude Code 的规则系统包括指令规则和权限规则，分别位于 .claude/rules/ 和 .claude/settings.json 中，两者协同构成完整的行为约束体系。
tags:
  - clippings
  - claude-code
  - rules
lark_doc_url: https://my.feishu.cn/docx/VTWZd0q98oaObhxTFw9clxq0n9f
---

> [!abstract] 释题
> Claude Code 的"规则"不止一种——指令规则告诉 Claude 该怎么做，权限规则告诉 Claude 能做什么。前者是员工手册，后者是门禁系统，两套规则协同运作，才构成完整的行为约束体系。
...
假设你有一个 React + Express + PostgreSQL 的全栈项目，CLAUDE.md 膨胀到了 600 行。我们如何拆分呢？
...
指令层有指令规则（CLAUDE.md、.claude/rules/），管控层有权限规则（settings.json、Hooks、CLI 参数）。
...
- ❌ `.claude/rules/security.md` 里写："禁止执行 rm -rf 命令" → Claude 可能遵守，也可能忘记。这是软约束。
- ✅ 在 settings.json 的 deny 中写：`Bash(rm -rf *)` → 客户端硬拦截，Claude 连尝试的机会都没有。

**错误 2：在 settings.json 中写编码规范**

- ❌ settings.json 无法表达"用 TypeScript 写代码"这样的指令 → 它只能控制 allow/deny，不能传递知识
- ✅ 在 CLAUDE.md 或 `rules/*.md` 中写编码规范

**错误 3：rules 文件之间互相矛盾**

- ❌ coding.md 说"缩进用 2 空格"，frontend.md 说"缩进用 4 空格" → Claude 会困惑，行为不可预测
...
- ❌ 期望子代理自动遵守 `.claude/rules/` 中的规范 → 子代理看不到主对话的任何记忆文件
...
> [!note] 我的思考：Rules vs Skills —— 跨产品对比视角
>
> **一、从 Cursor 迁移过来的体感**
>
> Cursor 的 `.cursorrules` 很早就支持了条件加载（按文件类型匹配），而 CC 的 `paths` 字段是后来才加的。从时间线看，CC 的 rule 系统确实像是被 Cursor 倒逼出来的——发现打不过，于是贡献了 Skills 这个规范出来，现在两家都是两套机制并存。
>
> 这个判断的注脚是：压力之下，反而爆发出来一个崭新的小宇宙。
>
> **二、佳哥的澄清：加载时机和生效范围是关键区分**
>
> • Rules：被动加载，环境触发，Always-once-loaded——当碰到 X 类文件时注入规范
> • Skills：主动激活，意图触发，On-demand——当用户需要做 X 时加载能力包
>
> 它们不在功能强弱，而在触发机制不同。
>
> **三、上下文压缩后的差异——Skills 目前无法覆盖 Rules 的核心场景**
>
> Rules 和 CLAUDE.md 是基础系统指令，注入 System Prompt。上下文压缩后，下一轮请求会**重新匹配 paths 并重新注入完整内容**。
>
> Skills 没有这个机制。长上下文时，压缩后 Skills 的规则约束可能就丢失了。
>
> 这意味着：**Rules 更适合那些"丢了就会出事故"的底线约束**（安全规范、数据库操作规范等），因为压缩不会丢失它们。这是 Skills 在机制层面目前还覆盖不了 Rules 的地方。
>
> **四、终极判断**
>
> Rules 和 CLAUDE.md 是历史产物。从渐进加载的角度看，它们本质上就是**特殊的、内建的、不可分发的 Skills**——只是 CC 还没把它们定义为一种 Skill 类型。
>
> 如果哪天 CC 把 Rules 统一成 `type: rule` 的 Skills（可条件加载、压缩不丢失、可分发），那就是机制收敛的时刻。
