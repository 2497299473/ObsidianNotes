...
aliases:
  - Skills专题总结
  - Claude Code Skills加餐
status: 📚 已整理
lark_doc_url: https://my.feishu.cn/docx/CgmgdNQTYo6Ej0xc0yPcjD3fnVc
---

> 来源：黄佳 · Claude Code 工程化实战 - 极客时间
...
![Skills 六讲全景](https://static001.geekbang.org/resource/image/ab/38/abf8caafb622051f92cfb6834b354e38.jpg?wh=3376x1836)

6 篇讲义、Demo 项目对应关系如下。

![讲义与项目对应](https://static001.geekbang.org/resource/image/70/cb/70b1bb13a5b14cba0b3e8d667ecc1acb.jpg?wh=3103x1821)
...
- 第 9 讲：知识的结构与触发 → 知识怎么写、什么时候被加载
- 第 10 讲：知识的操作化 → 知识怎么变成可执行的标准流程
- 第 11 讲：知识的经济学 → 如何用最少的 token 传递最多的知识

这一层解决的是**知识管理问题**。你的团队有大量专业知识（编码规范、架构约定、审查标准、部署流程），怎么让 AI 在需要时精准获取，不需要时不浪费上下文？

**第二层：系统集成（第 12-14 讲）——让知识驱动整个智能体系统**

- 第 12 讲：知识 × 智能体 → Skill 怎么和 SubAgent 组合成领域专家
- 第 13 讲：知识在架构中的位置 → 五层架构全景定位，从 SOP 到专家系统
- 第 14 讲：知识的平台逃逸 → 声明式 + 自包含 + 知识本位 = 跨平台标准
...
![演进路径](https://static001.geekbang.org/resource/image/94/d9/94b211e58162aef4ae0e73df4373e3d9.jpg?wh=3992x883)
...
第 13 讲提出了四种设计模式：**模板驱动**（约束输出格式）、**脚本增强**（封装确定性逻辑）、**分层知识**（按频率分级加载）和**工具隔离**（权限是最重要的设计决策）。

这里我设计了一张图，希望通过更加工程化的视角帮你建立选型直觉。

![四种设计模式](https://static001.geekbang.org/resource/image/af/6d/af1554b2a4a55999c008a751c33e2b6d.jpg?wh=3930x1996)
...
1. `find src -name "*.java" -newer refactor-baseline | head -50` → 动态获取最近修改的文件
...
完整的遗留代码现代化流水线，整体知识流如下。

![知识流](https://static001.geekbang.org/resource/image/e7/43/e76520fce796dea60ce622240afc0543.jpg?wh=1897x1446)
...
| 讲次 | 方法论 | 解决的问题 |
|------|--------|-----------|
| 第 9 讲 | description 三要素（做什么 + 怎么做 + 什么时候用） | "Claude 什么时候加载这个 Skill" |
| 第 10 讲 | 七步设计清单（动作→触发→权限→上下文→安全网→隔离→模型） | "怎么从零设计一个任务型 Skill" |
| 第 11 讲 | 知识 ROI 框架（关键知识常驻 / 补充知识按需 / 冗余知识排除） | "知识太多怎么组织" |
| 第 12 讲 | 两个原子方向（Skill 管 Agent / Agent 载 Skill）× 三种组合模式 | "Skill 和 SubAgent 怎么配合" |
| 第 13 讲 | 企业本体论映射（遇到设计决策，问"在企业里谁管这个"） | "用哪个机制解决哪类问题" |
| 第 14 讲 | Push vs Pull 框架（高频 Push 到 CLAUDE.md / 低频 Pull 到 Skills） | "这条知识放在哪里最高效" |
...
最后，佳哥给出 Skills 的终极定义版本——**Skills 是 Claude Code 架构中的知识层，它把人类组织几十年积累的知识管理经验（SOP、专家系统、组织学习）技术化映射到 AI Agent 架构中。**

它不是被动的文档，而是有触发条件、有执行流程、有质量标准、有工具约束、有自动检查的**可操作知识体系**。

- 它与 **Tools**（行动层）的关系是共生：约束、编排、反哺。
- 它与 **SubAgents**（智能体层）的关系是互补：知识注入 vs 任务委托。
- 它在**五层架构**中的位置是承上启下：向下约束工具，向上服务智能体。
...
选择你团队中一个**真实的知识管理痛点**，构建完整的 Skill 解决方案。
