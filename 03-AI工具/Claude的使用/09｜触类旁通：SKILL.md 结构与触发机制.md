...
tags:
  ...
  - skills
  - agent
description: Skills 是可被语义触发的能力包，包含领域知识、执行步骤与约束条件。本文从第一性原理出发，深入 Skills 的触发机制、两大类型（参考型/任务型）、SKILL.md 文件结构、frontmatter 字段详解，以及 description 写作公式。
lark_doc_url: https://my.feishu.cn/docx/HXnMd1QrSodD2sxB37yc3OLYn7e
---
...
![Agent 生态四大支柱](https://static001.geekbang.org/resource/image/b0/6f/b06f52d3699f1946e3ab28c5a03f286f.jpg?wh=1600x936)
...
- **Skills** 回答的是**怎么做，以及何时做**。它不是工具，也不是分工机制。它是一种**可操作知识结构**。
...
!["人调度模型" vs "模型调度能力"](https://static001.geekbang.org/resource/image/ef/67/ef6a39d41507e12437a2a472585df267.jpg?wh=3500x2002)
...
![企业本体论映射](https://static001.geekbang.org/resource/image/87/90/879040ed2fa7cc8d882be67205c65490.jpg?wh=2726x1996)
...
![Skills 生态全景](https://static001.geekbang.org/resource/image/be/b0/bee449cf0d56861d68e60105423fa7b0.jpg?wh=9843x6302)

这就是为什么 Skills 在 Agent 时代具有特殊地位。它既不是单纯的工具层扩展，也不是应用层封装。它是认知层的调度结构，是企业本体论在模型世界中的映射。

![四层架构](https://static001.geekbang.org/resource/image/8d/06/8dda11236e0e3ced0d6a5fc23688cc06.jpg?wh=2955x1041)
...
![Tools, Skills, Agents 和企业本体论体系](https://static001.geekbang.org/resource/image/6b/45/6b62767324fbab87c0337051c3b88c45.png?wh=1922x1048)

上面这张图真正希望你关注的，不是四个并列组件，而是四种不同层级的存在方式：

- **工具**回答"能做什么"
- **技能**回答"该怎么做"
- **智能体**回答"什么时候做什么，以及由谁来做"
- **本体论**回答"在什么世界里做"
...
![两种触发方式](https://static001.geekbang.org/resource/image/fe/67/fef174f446c78029759466acc16e6667.jpg?wh=3241x1054)
...
![命令触发 vs 语义触发](https://static001.geekbang.org/resource/image/10/c9/10d56b4dd8987fff2b1932770ed4afc9.jpg?wh=3170x1009)
...
![Skill 触发流程](https://static001.geekbang.org/resource/image/1d/01/1dd11f886258814d259171fcf8yya301.jpg?wh=3110x1881)

假设你有 5 个 Skills，每个 SKILL.md 约 1000 tokens。

![渐进式加载 Token 节省](https://static001.geekbang.org/resource/image/6a/96/6a72a495cbd2c751f547c3701165f696.jpg?wh=2646x1023)
...
![两种触发方式的区别](https://static001.geekbang.org/resource/image/af/8d/af26205919a87b1ae379bf33d63be88d.jpg?wh=3106x1097)
...
![存放位置与优先级](https://static001.geekbang.org/resource/image/5e/c7/5e4866155259b24caa583cff8bbccdc7.jpg?wh=3461x1293)
...
![Monorepo Skills 自动发现](https://static001.geekbang.org/resource/image/84/2e/84484b7c0dd1a22fd85e361ccab7542e.jpg?wh=3051x822)
...
![参考型 vs 任务型](https://static001.geekbang.org/resource/image/34/aa/34a6c2yy6a6e645ea14093ed31388eaa.jpg?wh=3449x1277)
...
最大 64 字符，只能使用小写字母、数字、连字符，推荐使用动名词形式：`code-reviewing`、`api-documenting`、`bug-fixing`。如果省略了这个字段，则自动使用目录名（`.claude/skills/code-reviewing/` → name 为 `code-reviewing`）。

### description 字段

这是最重要的字段——它决定 Skill 何时被触发。这个字段应该包含两部分信息：这个 Skill 做什么，以及什么情况下使用它。如果省略了这个字段，系统会使用 Markdown 正文的第一段作为 description。
...
自动补全提示，为用户提供参数格式提示，在输入 `/skill-name` 时系统会自动补全显示：
...
这两个字段组合起来控制"谁能触发这个 Skill"。

![触发控制组合](https://static001.geekbang.org/resource/image/71/cc/71569d313a1aa33d72c0f7867d2ceecc.jpg?wh=3741x1446)

![触发矩阵](https://static001.geekbang.org/resource/image/8b/cc/8b1d56dccd2ea9a4954f60bf94213fcc.jpg?wh=2939x924)

### allowed-tools 字段

用来限制 Skills 被激活时 Claude 能使用的工具。Skills 支持的工具包括：

![allowed-tools 支持的工具](https://static001.geekbang.org/resource/image/71/b6/7139daee27a1cf343b46086528bb16b6.jpg?wh=2762x1724)
...
![执行环境配置](https://static001.geekbang.org/resource/image/2b/eb/2bb9637c7c3fyy09bf8197e3ae446aeb.jpg?wh=2987x1095)

### hooks——Skill 级别的 Hooks

可以为 Skill 定义仅在其生命周期内生效的 Hooks。
...
![设计思维框架](https://static001.geekbang.org/resource/image/e0/ef/e0326faee1ef96de7e9804b86b6b73ef.jpg?wh=3742x2206)
