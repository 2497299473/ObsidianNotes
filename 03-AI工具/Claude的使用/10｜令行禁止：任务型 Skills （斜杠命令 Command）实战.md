---
title: 10｜令行禁止：任务型 Skills（斜杠命令 /Command）实战
...
tags:
  ...
  - skills
  - commands
description: 任务型 Skill 是一种用于执行特定工作流程的技术，支持参数传递和动态上下文注入，能提高工程效率，减少重复对话模式。设计时需遵循七步设计清单，包括动作命名、触发者、权限、上下文、安全网、输出量和模型选择。
lark_doc_url: https://my.feishu.cn/docx/X7pld101roFd60xfGOVcg6IRn1g
---
...
早期，斜杠命令 `/Commands` 和 Skills 是两个独立组件。但在新版 Claude Code 中，Commands 已合并到 Skills，成为 Skills 的子集。
...
![](https://static001.geekbang.org/resource/image/56/77/56006a3192f86ace7d22dd6997c5cf77.jpg?wh=2958x1202)
![](https://static001.geekbang.org/resource/image/2e/10/2e8cf0fc8303b659ef23585d6bc10f10.jpg?wh=3123x1228)
...
Claude 实际收到的 Prompt（替换后）就拥有了完整上下文，这样 Claude 启动 `/pr-create "Add auth"` 时就可以直接生成 PR 标题和描述，无需额外再进行多一次工具调用。
...
`!`command`` 可以与 `$ARGUMENTS` 组合，在动态注入时使用参数值。`$ARGUMENTS` 参数会先被替换，再执行 `!`command``。这意味着用户输入会进入 shell 命令——因此**务必在 `allowed-tools` 中严格限制可执行范围**。
