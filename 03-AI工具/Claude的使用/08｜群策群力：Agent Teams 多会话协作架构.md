---
title: 08｜群策群力：Agent Teams 多会话协作架构
created: 2026-06-20
tags:
  - clippings
  - claude-code
  - agent-teams
  - sub-agents
description: Agent Teams 是 Claude Code 的实验性功能，通过团队协作模式让代理之间直接交流、互相挑战、协作推进，支持竞争假设、分层评审、模块化开发和规划-审批四种核心设计模式。
lark_doc_url: https://my.feishu.cn/docx/GmZodnZwwoMn4yxXUtDcKdhMnwh
---
...
- **Team Lead**：协调者，分配任务、综合结果
- **Teammates**：执行者，各自独立工作
- **Task List**：共享任务列表
- **Mailbox**：消息系统，用于 Teammate 间通信
...
- **完整模式**：显示所有 Teammates 的详细输出
- **摘要模式**：只显示 Lead 的综合摘要

完成工作后，Lead 会向 Teammate 发送关闭请求，Teammate 可以批准（优雅退出）或拒绝（并解释原因）。最后，全部工作收尾，Lead 会清理团队，移除共享的团队资源。需要注意，如果出现需要用户人工指示清理的情况，清理前先关闭所有 Teammates，而且应该只让 Lead 执行清理（Teammates 的 team 上下文可能不正确）。
...
阅读 `bug-report.md` 中描述的三个症状，然后创建一个 agent team 来调查问题。生成 4 个 investigator teammates：
...
要求所有 teammates 在完成初步调查后互相分享发现，并尝试挑战彼此的结论。最终综合所有发现，生成一份按照 `findings-template.md` 格式的调查报告。
...
Lead 会根据你提供的标准自主做出批准/拒绝决策。你影响 Lead 判断的方式是在 prompt 中给出明确的标准。
...
> [!note] 延伸思考：Agent 应该是进程还是函数？
> Agent Teams 用完即销毁——这引出一个更深层的设计问题：Agent 应该是**长期存活的进程**，还是**调用即生、返回即死的函数**？
>
> Claude Code 选择了**函数模型**——每个 SubAgent 和 Agent Team 都是一次性的。这不是技术限制，而是刻意设计。
>
> 常驻进程的优势是"记得上次说了什么"，但代价是：
> • 上下文不断膨胀
> • 历史偏见累积
> • token 持续消耗
>
> **函数模型 + 文件持久化**是一个更务实的方案：把值得记住的写成文件，把不值得记住的让它消失。
>
> 这其实就是人类专家的工作方式——你不会记得每一次代码审查的细节，但你会把反复出现的问题写进团队的 checklist。Skill 就是那个 checklist，`.claude/history/` 就是那本工作日志。

## Token 成本考量

Agent Teams 使用的 token 显著高于单会话或子代理，因为每个 Teammate 是独立的 Claude 实例，有独立的上下文窗口。消息通信消耗额外 token，而且团队越大，成本越高。
...
Agent Teams 让多个 Claude Code 实例作为团队协作，Teammates 可以互相通信，而不只是向主对话汇报。其核心组件包括：

| 组件 | 职责 |
|------|------|
| Lead | 协调者，分配任务、综合结果 |
| Teammates | 执行者，各自独立工作 |
| Task List | 共享任务，支持依赖关系 |
| Mailbox | 消息系统，Teammate 间通信 |

**Teams 最佳实践：**

1. **给 Teammates 足够的上下文。** Teammates 不继承 Lead 的对话历史，所以生成时要给足信息。
2. **合理拆分任务粒度。** 太小的话协调开销超过收益，过大则 Teammate 工作太久无检查点，增加返工风险。任务最好是自包含的单元，产出明确的交付物（一个函数、一个测试文件、一份审查报告）。每个 Teammate 5-6 个任务比较合适。
3. **避免文件冲突。** 两个 Teammates 编辑同一个文件会导致覆盖。拆分工作时确保每个 Teammate 拥有不同的文件集。
4. **从研究和审查任务开始。** 如果你刚接触 Agent Teams，从边界清晰、不写代码的任务开始，如审查 PR、调研技术方案、调查 bug 等。
5. **让 Lead 等待 Teammates 完成。** 有时 Lead 会在 Teammates 完成前就自己开始下一步工作。如果发现这种情况，可以在命令行提示"等待你的 teammates 完成任务后再继续"。

## 思考题

**1. 回顾你之前用子代理做的任务。有哪些场景如果用 Agent Teams 的"竞争假设"模式可能效果更好？**

可以在我的 consigliere 里面加入：针对 journal_agent 提出的建议，用竞争假设模式让多个 Teammates 各自持不同书籍/理论来辩论，看哪种理论能更好地解决问题，从而提出最合适的行为建议。

**2. 在"模块化开发"模式中，如果两个 Teammates 需要修改同一个接口文件（比如定义 API 契约），你会如何处理文件冲突问题？**

加入人工审核节点，让人来决定谁先谁后。或者事先在任务定义时就安排好优先级，哪个 Teammate 先写接口定义，另一个只读引用。

**3. 对比 Sub-Agents 的"流水线编排"和 Agent Teams 的规划-审批模式。两者解决的问题有什么不同？在什么场景下你会选择哪个？**

最大的不同在于子代理之间**能否通信**。当任务边界明确、一个任务开始的条件是上一个任务完成，用流水线；当需要各自商量以做出更好决策，用 Agent Team——比如前后端接口匹配，前后端 Teammates 需要就 API 契约达成一致。

**4. 假设你要用 Agent Teams 做一次技术方案评审——有三个 Teammates 分别代表"性能优先""可维护性优先""快速交付优先"三种立场。设计一下你会给 Lead 什么指令，让这场讨论产出高质量的决策。**

给 Lead 的指令应包含：我的偏好（如更看重哪一维度）、结束任务的明确标识（如"产出共识文档"或"无法达成共识则列出分歧点"）、我本地开发的基础环境要求、项目目录结构。

**5. 在"全栈 Bug 猎人"项目中，四个 bug 的级联关系是如何被发现的？如果用子代理（而非 Agent Teams）调查这些 bug，你预期会有什么不同的结果？具体到哪些关联会被遗漏？**

级联关系是子代理彼此竞争、从各自角度出发相互对照发现的。如果是子代理模式，各代理会在不同文件各自修改各自发现的错误，互不知晓对方的修复方案，可能导致 bug 越修越多，同时浪费大量 token。至于具体哪些关联会被遗漏——我不是开发人员，没有实际经验，无法判断。
